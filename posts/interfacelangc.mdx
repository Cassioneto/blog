...
getch() do grupo de funções da library citada acima lê o valor inteiro correspondente a tabela **ASCII** [link text](https://en.wikipedia.org/wiki/ASCII) . Segundo está vem as teclas das setas do teclado podem ser o nosso cursor( não esquecemos falamos de linhas de comando e não interface gráficas!).
...
```C
if((posição!=0)&&(menu==teclaEsquerda){
  posição-=1;
  }
else if((posição!=1)&&(menu==teclaDirecta)){
  posição+=1;
  }
```
...
if((espaco%2)==0)//a ver se o restante é par
{
